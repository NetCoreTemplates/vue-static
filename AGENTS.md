...
A full-stack .NET 10 + Vue + Vite template combining ServiceStack backend with Vite static site generation. Uses ASP.NET Core Identity for auth, OrmLite for application data, and Entity Framework Core for Identity data management.
...
Inside a Vue Component use `useClient()` to resolve a Service Client. The `ApiResult` can hold **loading**, **failed** and **successful** API Response states, e.g:
...
```typescript
const submit = async (e:HTMLFormElement) => {
...
The `@servicestack/vue` [component library](https://docs.servicestack.net/vue/) have several components to simplify UI generation. 
...
### useClient and Input Components

The `useClient` can be used to inject an APIs Error `ResponseStatus` down to all `@servicestack/vue` Input components.
...
async function onSubmit(e: Event) {
  const {userName, password} = serializeToObject(e.currentTarget as HTMLFormElement)
...
- All other routes → Vue SPA (via fallback in dev/prod)
